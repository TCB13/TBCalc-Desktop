...
"C:\Program Files (x86)\Windows Kits\10\bin\10.0.19041.0\x64\rc.exe" /fo "%src_dir%/windows.res" "%src_dir%\windows.rc" 

:: Dynamic Linked Build
::echo Building TBCalc.exe (x64)
::cl /I "%script_dir%\microsoft.web.webview2.%nuget_version%\build\native\include" ^
::	"%script_dir%\microsoft.web.webview2.%nuget_version%\build\native\x64\WebView2Loader.dll.lib" ^
::	"%src_dir%\dll\x64\webview.lib" ^
::	/std:c++17 /EHsc "/Fo%build_dir%"\ ^
::	"%src_dir%\main.cc" /link "/OUT:%build_dir%\TBCalc.exe" "%src_dir%\windows.res" || exit \b
...
cl /I "%script_dir%\microsoft.web.webview2.%nuget_version%\build\native\include" ^
	...
	"%src_dir%\dll\x64\webview.lib" ^
	/std:c++17 /EHsc /O2 /GS "/Fo%build_dir%"\ ^
	"%src_dir%\main.cc" /link "/OUT:%build_dir%\TBCalc.exe" "%src_dir%\windows.res" || exit \b

echo Copying TBCalc source code
copy  "%script_dir%\..\..\TBCalc" "%build_dir%\tbcalc"
